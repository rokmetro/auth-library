...
func TestAuthService_GetServiceReg(t *testing.T) {
	...
	testServiceReg := authservice.ServiceReg{"test", "https://test.rokmetro.com", nil}
	authServiceReg := authservice.ServiceReg{"auth", "https://auth.rokmetro.com", authPubKey}
...
func TestAuthService_ValidateServiceRegistration(t *testing.T) {
	...
	tests := []struct {
		...
		{"no error on registration found", args{"https://test.rokmetro.com"}, serviceRegsValid, false},
		{"error on registration not found", args{"https://test.rokmetro.com"}, serviceRegsMissing, true},
		{"error on wrong registration host", args{"https://test2.rokmetro.com"}, serviceRegsValid, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
-----END RSA PUBLIC KEY-----`
	...
	testServiceReg := authservice.ServiceReg{"test", "https://test.rokmetro.com", pubKey}
	testServiceRegNoKey := authservice.ServiceReg{"test", "https://test.rokmetro.com", nil}
	testServiceRegWrongKey := authservice.ServiceReg{"test", "https://test.rokmetro.com", wrongKey}

	authServiceReg := authservice.ServiceReg{"auth", "https://auth.rokmetro.com", nil}
