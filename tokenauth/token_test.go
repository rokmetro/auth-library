...
func TestTokenAuth_CheckToken(t *testing.T) {
	...
	tests := []struct {
		...
		{"return error on valid token", args{"token", "access"}, nil, true, "failed to parse token"},
		{"return error on retry invalid token", args{"invalid retry token", "access"}, nil, true, "initial token check returned invalid, error on retry"},
		{"return claims after refresh", args{"valid token after refresh", "access"}, &tokenauth.Claims{}, false, ""},
		{"return eror on expired token", args{"expired token", "access"}, nil, false, "token has expired"},
	}
